	...
	public AnswerScreen() {
		...
		for(int i = 1; i <= totalQuestion.length; i++){
			...
			question = new Question(_texQuestion, new Rectangle(MainGame.x, MainGame.y, WIDTH, HEIGHT),
					                              new Rectangle(120 + MainGame.x, 725 + MainGame.y, 0, 0),
			...
			for(int j = 1; j <= totalAnswer; j++){
				...
				int x = position[0];
				int y = position[1];
				answers.add(new Answer(_texAnswer, new Rectangle(x + MainGame.x - ANSWER_X_PLUS, y + MainGame.y - ANSWER_Y_PLUS, _texAnswer.getWidth(), _texAnswer.getHeight()),
                        						   new Rectangle( x + MainGame.x, y + MainGame.y, 0, 0),
                        						   _fontAnswer, j, answerProperties.nextToken(),
                        						   new MyActor(new Rectangle(x + MainGame.x + POINT_X_PLUS, y + MainGame.y - POINT_Y_PLUS, 0, 0), _fontPoint, answerProperties.nextToken())));
			}
